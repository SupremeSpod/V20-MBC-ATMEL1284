...
/*-----------------------------------------------------------------------*/
/* Get cluster# based on directory entry                                 */
...
static FRESULT dir_rewind(DIR *dj) 
{
  ...
  if (_FS_FAT32 && !clst && (_FS_32ONLY || fs->fs_type == FS_FAT32)) 
  {
...
static FRESULT dir_find(DIR *dj, BYTE *dir) 
{
  ...
  {
    /* Read an entry */
    res = disk_readp(dir, dj->sect, (dj->index % 16) * 32, 32) ? FR_DISK_ERR : FR_OK;
    if (res != FR_OK) 
    {
      break;
    }
...
/*-----------------------------------------------------------------------*/
#if _USE_DIR
static FRESULT dir_read(DIR *dj, BYTE *dir) {
  FRESULT res;
  BYTE a, c;
...
/*              (NULL:Forward data to the stream)                        */
...
/*                      (0:Finalize the current write operation)         */
